"""SWIN + FAISS retrieval classifier (mgrandhi — uses teammate repo assets at root level).

Assets at the teammate repo root:
  - swin_model_assets/                    : the SWIN encoder
  - swin_processor_assets/                : the SWIN preprocessor
  - swin_faiss_index.bin                  : the FAISS index
  - swin_faiss_indexed_image_paths.csv    : index -> image_path mapping
  - train_product_category_58.csv         : image_path -> label mapping
"""
...
# Teammate repo root — assets live at the repo root, not under mgrandhi/
...
class SwinFaissClassifier:
    def __init__(
        self,
        model_dir: str | Path = REPO_ROOT / "swin_model_assets",
        processor_dir: str | Path = REPO_ROOT / "swin_processor_assets",
        index_path: str | Path = REPO_ROOT / "swin_faiss_index.bin",
        image_paths_path: str | Path = REPO_ROOT / "swin_faiss_indexed_image_paths.csv",
        labels_csv: str | Path = REPO_ROOT / "train_product_category_58.csv",
    ):
